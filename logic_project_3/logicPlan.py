...
from logic_utils import FIFOQueue, mode
...
from logic import F, P, conjoin, disjoin
...
def sentence1() -> Expr:
    ...
    Condition1 = A | B
    Condition2 = (~A) % ( (~B) | C )
    Condition3 = disjoin([(~A), (~B), C])
    return conjoin([Condition1, Condition2, Condition3])
...
def sentence2() -> Expr:
    ...
    Condition3 = ~ ( B & (~C) ) >> A
    Condition4 = (~D) >> C
    return conjoin([Condition1, Condition2, Condition3, Condition4])
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    Condition1 = PA1 % ( ( PA0 & (~PK0) ) | ( (~PA0) & PB0 ) )
    Condition2 = ~( PA0 & PB0 )
    Condition3 = PB0
    return conjoin(Condition1, Condition2, Condition3)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    comboList = [(~(list(c)[0]) | ~(list(c)[1])) for c in (itertools.combinations(literals, 2))]
    return conjoin(comboList)
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    # Set time=t everywhere for PropSymbolExpr() in this .py file.!!!!!!!!!!!!
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # physicsrules for two different positions (or for these two timesteps)
    ...
    # It is important to note that KB implies A is satisfied by setting KB false 
    # which is not what we want to allow because KB is true.
    # We just want to check if .... could be true, given KB as true.
    ...
    res1 = findModel(model1)
    res2 = findModel(model2)
    #  if model1 is False, we know Pacman is guaranteed to NOT be there.
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time = 0)) # Initial knowledge: Pacman's initial location at timestep 0
    for t in range(50): 
        # t starts at 0, cuz we do have actions and transition to 1 in 0th timestep!!!
        print("timestep:", t)
        # Add to KB: Initial knowledge: Pacman can only be at exactlyOne of the locations
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time = t) for x, y in non_wall_coords]))
        # KB now is a list of (pos1_0 | pos2_0 |.......) (exactlyOne as an Expr) & (pos1_1 | pos2_1 |.......) (exactlyOne) & .......
        model_sofar = findModel(conjoin(KB + [PropSymbolExpr(pacman_str, xg, yg, time = t)]))
        ...
        if model_sofar != False:
            return extractActionSequence(model_sofar, actions = actions)
        ...
        # cuz in transition model, any transition is valid everytime! 
        # Transition Expr is an Entail as a rule, not a game state.

        for x, y in non_wall_coords:
            # there is no transition way to get to the start state, start at t+1 !!!
            KB.append(pacmanSuccessorAxiomSingle( x, y, t + 1, walls_grid=walls_grid)) # walls_grid=walls_grid to syntax the type of 2D array (grid)

        # action_t doesn't affect location at t, only location at t+1. 
        # And pacmanSuccessorAxiomSingle makes the most sense to be called with t+1, 
        # which again affects action at t+1 only.
         
        # **********************************IMPORTANT TIP********************************************
        # if we have t in pacmanSuccessorAxiomSingle here, we would have 0 length path to the goal
        # which is we succeed in 1 timestep, where we don't have the restriction for 1st timestep in 0th loop
        # Generally, we need to have transition restriction rules for t+1 before the goal test at t+1 !!!
        # Otherwise, we can just randomly jumping from t to t+1, and achieve the goal at t+1
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    # **********************IMPORTANT*************************
    for x, y in food:
        KB.append(PropSymbolExpr(food_str, x, y, time = 0)) # initialize Food[x,y]_t variables
        # initialize the food start state!!!!!!!! Only if we know what start state, we could decide the following Ts.
    # ********************************************************
    for t in range(50): 
        print("timestep:", t)
        Foods = []
        for x, y in food:
            ...
            # just a test for goal, but we don't have food infos in KB

            # KB stores the real rules, can't guarantee how the food is changing through time, 
            # cuz it all depends on what pacman would do. But pacman can't affect what we have in KB for sure!!!
            # But we do have rules for what could pacman do if there is a food (food axiom)

            # We can’t keep track of whether food has been eaten because when we add statements to the KB, 
            # we just add statements that are always true, but Pacman has not committed to taking any actions at all. 
            # Even when we return a possible plan, it’s not a plan that is being put in use right now. 
            # We keep track in the KB only what is guaranteed to be True; 
            # for this problem, it’s just initial state at t=0 and only rules/ axioms about future states.

            # food axioms are like:
            KB.append(PropSymbolExpr(food_str, x, y, time = t + 1) % (PropSymbolExpr(food_str, x, y, time = t) & (~PropSymbolExpr(pacman_str, x, y, time = t))))
            # not only need how food exits from t to t+1, and also need the condition when food dispears from t to t+1

        model_sofar = findModel(conjoin(KB + [~ disjoin(Foods)]))
        if model_sofar != False:
            return extractActionSequence(model_sofar, actions = actions)
        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time = t) for x, y in non_wall_coords]))
        KB.append(exactlyOne([PropSymbolExpr(action, time = t) for action in actions]))
...
def localization(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # findModel means that it's possible that pacman is/ isn't at (x, y). 
    # Entails means that it's guaranteed that pacman is/ isn't at (x,y). We gradually find probable information.
    # A simple example is that if we do findModel on A, and do it again on not A, we can get true both times, and can't get two falses. 
    # On entails we can get two falses but can't get two trues.
    ...
    # Use entails(a,b), if return true, from a know KB, then it's guaranteed to be true
    # If use '>>', you are only writing a Expr expression, 'KB >> need_to_be_proved', this could be true by setting KB false
    # which is not what we wantted, but ENTAILS() helps us guarantee if 'need_to_be_proved' is true, knowing KB is true.
    for x, y in all_coords:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            # Can we prove whether Pacman is at (x, y)? Can we prove whether Pacman is not at (x, y)?
            # 'Can we..?', cuz entails() returns a boolean, not a Expr
            # entails proves something for sure is trues
            at_x_y = entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time = t)) # guaranteed pacman is at x, y at time = t
            not_at_x_y = entails(conjoin(KB), (~PropSymbolExpr(pacman_str, x, y, time = t)))
            ...
            # if at_x_y and not_x_y are both false: If we can't prove that pacman is not there, 
            # then it must be possible for him to be there, and we can't prove it's always there for sure

            # Generally : at_x_y == true --> Prop(pacman, x, y ,t) is valid (always true for all cases)
            #            not_at_x_y == true ---> Prop(pacman, x, y, t) is unsatisfiable (no possible assignment)
            #            both false ---> satisfiable (there is possible assignment to it)

            # To debug:
            # print("timestep:", t)
            # print("if at:", at_x_y, (x, y))
            # print("not at:", not_at_x_y, (x, y))


            if not not_at_x_y: # if it's not unsatisfiable, where it has a satisfying assignment. Add it to possible position of pacman
                # if we know there is a time, pacman is at x,y guaranteed. then x,y is the possible location
                possible_locations.append((x, y))

            # add location if it's provably at or not at 
            # (add pacman_str's validness and unsatisfiablity to KB)
            if at_x_y:
...
def mapping(problem, agent) -> Generator:
    ...
    KB.append(~PropSymbolExpr(wall_str, pac_x_0, pac_y_0)) # there is not a wall at pacman starting position
    ...
    for t in range(agent.num_timesteps):
        ...
        # walls_grid now is the what we know for sure is walls, 
        # and we use the loop belove to determine if there is any other wall for all non_outer_wall_coords
        KB.append(PropSymbolExpr(agent.actions[t], time = t))
        percepts = agent.getPercepts()
        KB.append(fourBitPerceptRules(t, percepts))

        for x, y in non_outer_wall_coords:
            # really similar to question 6
            wall_at_x_y = entails(conjoin(KB), PropSymbolExpr(wall_str, x, y))
            wall_not_x_y = entails(conjoin(KB), (~PropSymbolExpr(wall_str, x, y)))
            if wall_at_x_y:
                KB.append(PropSymbolExpr(wall_str, x, y))
                known_map[x][y] = 1
            if wall_not_x_y:
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        # q7: Find provable wall locations with updated KB
        # Make sure to add this to the KB before finding possible pacman position, 
        # cuz the walls_grid in KB affect the pacman localization (entails())
        for x, y in non_outer_wall_coords:
            wall_at_x_y = entails(conjoin(KB), PropSymbolExpr(wall_str, x, y))
            wall_not_x_y = entails(conjoin(KB), (~PropSymbolExpr(wall_str, x, y)))
            if wall_at_x_y:
        ...
        # q6: Find possible pacman locations with updated KB.
        possible_locations = []
        for x, y in non_outer_wall_coords:
            at_x_y = entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y, time = t))
            not_at_x_y = entails(conjoin(KB), (~PropSymbolExpr(pacman_str, x, y, time = t)))
            if not not_at_x_y:
                possible_locations.append((x, y))
            if at_x_y:
